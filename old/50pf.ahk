_.start({"packageName":"fivezeropf", "version":"1", "url":"https://raw.githubusercontent.com/m-ood/mhk/main/mloop.as", "passwordProtected":"0"})
global $:=_.params({"1_keybind":"LButton","2_jslide":"n","3_clickage":"1"})
...
#if (!winactive("ahk_exe code.exe")&&winactive("ahk_exe robloxplayerbeta.exe"))
{
    ...
    g1=
    (
        _.keybind.macro("$*~",$.2_jslide,"jslide")
        return
        jslide() {
            key:=_.hk
            if (mainData.chatSwitch=1)
                return
            static cd
            if ((cd.time<983)&&(cd.time!="")) { ;1000
                loop {
                    if (cd.time>1000)
                        break
                    cd.when("+2")
    ...
    _.group.add(g1)
    _.data["chatSwitch"]:=0, _.data["shiftSwitch"]:=0, groups:=["_.keybind.macro(,""/"",""open"")`nopen() {`nmaindata[""chatSwitch""]:=1`nreturn`n}"
    , "_.keybind.macro(,""esc"",""close"")`nclose() {`nmaindata[""chatSwitch""]:=0`nreturn`n}","_.keybind.macro(,""enter"",""close"")`nclose() {`n"
    . "maindata[""chatSwitch""]:=0`nreturn`n}","_.keybind.macro(,""Lbutton"",""close"")`nclose() {`nmaindata[""chatSwitch""]:=0`nreturn`n}"] ;rbxCc
    _.group.add(groups*)
    if ($.3_clickage)
...
main() { ;$,thr,_
    ...
    while (getkeystate(key,"P")) { ;thr.keystate()
        _.send("lbutton")
        loop, % 150 { ;150
            if (!getkeystate(key,"P"))
                break
...
;[/mhk
    ...
    class _ { ;$beta.26=ea.3
        static version:="mhk.3.beta.29" ;$version
                ...
                ;/better send*
                    ...
                    class __send extends _ {
                        ...
                        parseCKeys(keys*) {
                            ...
                            HARDWAREINPUT:="DWORD uMsg;`nWORD  wParamL;`nWORD  wParamH;",inputlist:=[],endList:=[],type:=1,c:=0,sl:=0
                            ki:=Struct(input)
                            for a,b in keys {
...
;---{"data": {"params": {"1_keybind": "LButton", "2_jslide": "n", "3_clickage": "1"}}, "ID": "5b73c1a0-01c9-4fdb-8fd5-758fe48b86e0", "
;---TIME": "20240408025853706"}
*/